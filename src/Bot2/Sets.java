...
public class Sets {
    ...
    Food add(String name, String tags, int group, int cost) {
        ...
        Food f5 = new Food("Самурай 320р.", "ролл, самурай, креветка тигр,еда,еды, лосос, помидоры", 1, 320, foods.length);
        ...
        Food f14=new Food("Сок Яблочный 60р.", "напит, пить, сок,яблочный", 4, 60, foods.length);
        ...
        Food f19=new Food("Сок Томатный 60р.", "напит, пить, сок,томат", 4, 60, foods.length);
        ...
        Food f21=new Food(" Анекдот Охотник-добытчик: Муж возвращается с охоты.\n" +
                "— Как поохотился дорогой?\n" +
                "Нормально, месяц не будем покупать мясо.,\n" +
                " — Ты убил лося?\n" +
                "Нет пропил зарплату.","Анекдот, Охотник-добытчик, смешной", 5, 160, foods.length );
        Food f20=new Food("Сок Папайя 60р.", "напит, пить, сок,папайя", 4, 60, foods.length);
        add(f1);add(f2);add(f3);add(f4);add(f5);add(f6);add(f7);add(f8);add(f9);add(f10);add(f11);add(f12);add(f13);add(f14);add(f15);
        add(f16); add(f17);add(f18);add(f19);add(f20);add(f21);
