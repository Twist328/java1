        ...
        if (message != null && message.hasText()) {
            ...
            switch (message.getText()) {
                ...
                case WHAT_THE_DATE_REQUEST:
                    ...
                    break;
                case GBP:
                    sendMsg(message, " Курс GBP (ЦБ РФ) 107,4 " + (LocalDateTime.now().format(DateTimeFormatter.ofPattern("СЕГОДНЯ   " + "dd-MM-YYYY ")) + (LocalDateTime.now().format(DateTimeFormatter.ofPattern("HH:mm ")) + Emoji.GRINNING_FACE_WITH_SMILING_EYES)));
                    break;
                case JPY:
                    sendMsg(message, "Курс 100 JPY (ЦБ РФ) 70,76\n" + (LocalDateTime.now().format(DateTimeFormatter.ofPattern("СЕГОДНЯ  " + "dd-MM-YYYY ")) + (LocalDateTime.now().format(DateTimeFormatter.ofPattern("HH:mm ")) + Emoji.GRINNING_FACE_WITH_SMILING_EYES)));
                    break;
                case EUR:
                    sendMsg(message, " Курс EUR (ЦБ РФ) 91,34 " + (LocalDateTime.now().format(DateTimeFormatter.ofPattern("СЕГОДНЯ   " + "dd-MM-YYYY ")) + (LocalDateTime.now().format(DateTimeFormatter.ofPattern("HH:mm ")) + Emoji.GRINNING_FACE_WITH_SMILING_EYES)));
                    break;
                case USD:
                    sendMsg(message, " Курс USD (ЦБ РФ) 77,77 " + (LocalDateTime.now().format(DateTimeFormatter.ofPattern("СЕГОДНЯ   " + "dd-MM-YYYY ")) + (LocalDateTime.now().format(DateTimeFormatter.ofPattern("HH:mm ")) + Emoji.GRINNING_FACE_WITH_SMILING_EYES)));
                    break;
                case HELP:
