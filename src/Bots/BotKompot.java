...
class PizzaBotKompot extends ProgwardsTelegramBot {
    ...
    public String processMessage(String text) {
        ...
        if (foundCount() == 1) {
            ...
            if (checkLastFound("Как сам"))
                return "О Привет! Пока сам! \n Что желаешь? " + menu;
        ...
        return "Я не понял, возможно у нас этого нет, попробуй сказать по другому." + menu;
            ...
            public static void main (String[]args){
                ...
                PizzaBotKompot bot = new PizzaBotKompot();
                ...
                bot.addTags("Пицца пепперони", "пепперони, пицц, салями, колбас, сыр, кетчуп, помидор");

                bot.addTags("Торт тирамису", "десерт, торт, тирамису, маскарпоне, бисквит");
                bot.addTags("Торт медовик", "десерт, торт, мед, бисквит");
                bot.addTags("Эклеры", "десерт, эклер, заварной, крем, тесто");
                ...
                bot.addTags("Кофе Американо", "американо, молоко");
                bot.addTags("Кофе Эспрессо", "эспрессо");
                bot.addTags("Кофе Капучино", "капучино, с молоком");
  ...
  /*static Menu menu;
   ...
   /* @Override
    ...
    public static void main(String[] args) {
        ...
        // инициализируем бота
        PizzaBotKompot bot = new PizzaBotKompot();
        bot.username = "PizzaBotKompot"; // не стал создавать нового бота
        bot.token = "1156742283:AAFI-S2YFXccCEwKA7j5pKhup8QfuCr3KDg";
